...
// maxRetriesPerRequest: null is required by BullMQ — without it the worker
// throws "BLMOVE" errors on long-lived connections
...
export async function scheduleAlerts(
  ...
  if (data.return_deadline_days !== null) {
    ...
    const sevenDayBefore = new Date(
      deadlineDate.getTime() - 7 * 86400000
    );
    const oneDayBefore = new Date(
      deadlineDate.getTime() - 1 * 86400000
    );

    const sevenDelay = sevenDayBefore.getTime() - Date.now();
    if (sevenDelay > 0) {
      jobs.push({
        jobData: {
          receiptId,
    ...
    const oneDelay = oneDayBefore.getTime() - Date.now();
    if (oneDelay > 0) {
      jobs.push({
        jobData: {
          receiptId,
  ...
  if (data.warranty_months !== null) {
    const warrantyDate = addDaysToDate(
      purchaseDate,
      data.warranty_months * 30
    );
    const thirtyBefore = new Date(
      warrantyDate.getTime() - 30 * 86400000
    );
    const sevenBefore = new Date(
      warrantyDate.getTime() - 7 * 86400000
    );

    const thirtyDelay = thirtyBefore.getTime() - Date.now();
    if (thirtyDelay > 0) {
      jobs.push({
        jobData: {
          receiptId,
          ...
          scheduledFor: thirtyBefore.toISOString(),
    ...
    const sevenDelay = sevenBefore.getTime() - Date.now();
    if (sevenDelay > 0) {
      jobs.push({
        jobData: {
          receiptId,
          userPhone,
          alertType: "warranty_expiry",
          scheduledFor: sevenBefore.toISOString(),
          payload: { daysRemaining: 7, store_name: data.store_name },
        },
        delayMs: sevenDelay,
      });
  ...
  for (const { jobData, delayMs } of jobs) {
    ...
    log(
      `Scheduled ${jobData.alertType} alert for receipt ${receiptId} ` +
        `in ${Math.round(delayMs / 86400000)} days`
    );
  }
}
